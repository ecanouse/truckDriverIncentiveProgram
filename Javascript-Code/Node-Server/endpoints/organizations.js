module.exports = function(app, connection){
    app.get('/getOrganizations', (req, res) => {
      ...
      const orgs_query = `SELECT o.sponsorID, orgName from new_schema.SPONSOR_ORG o INNER JOIN new_schema.USER_SPONSOR_REL r where r.uID=${userid} AND o.sponsorID = r.sponsorID;`;
    ...
    app.get('/getAllOrganizations', (req, res) => {
      const orgs_query = `SELECT sponsorID, orgName from new_schema.SPONSOR_ORG;`;
      connection.query(orgs_query, function(err, result) {
        if(err){
          console.log(err);
        }else{
